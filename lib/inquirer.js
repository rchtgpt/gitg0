...
const { recIssueType } = require('./funcs/rec-iss-type');
        ...
        inquirer.prompt([{
            ...
            .then(
                answer1 => {
                    ...
                    //         return
                    //     }
                    //     // conf.selected_commit_type = issue['issueType'];
                    ...
                    reader.jsonReader('./.gitgo', (err, conf) => {
                            ...
                            console.log(emoji.get(':smiley_cat:'), chalk.bold('Github Issue Title:'), title)
                        })
                    })
                    .then(() => {
                        // inquirer.prompt([{
                ...
                ans => {
                    if (ans['guidelines'] === "Type your own, if multiple, separate with commas") {
                        inquirer.prompt([{
                            // expects issue number as response
                            type: 'string',
                            message: 'Type your own guidelines, if multiple, separate with commas\n',
                            name: 'commit'
                        }]).then(
                            ans1 => {
                                reader.jsonReader('./.gitgo', (err, conf) => {
                                ...
                                inquirer.prompt([{
                                    // expects issue number as response
                                    type: 'string',
                                    message: 'Would you be using emojis in your commit messages?(y/n)',
                                    name: 'emojis'
                                }]).then(
                                    ans2 => {
                                        var emo = true;
