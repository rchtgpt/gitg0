...
const {
  suggestCommitMsg
} = require("./funcs/commit-gen");
const {
  branchName
} = require("./funcs/generate");
var titleOfIssue;
let labelsOfIssue = [];
const fs = require("fs");
const {
      ...
      .then(async function (response) {
        for (repo in response.data) {
          if (
            ...
            axios.get(finalIssueLink).then(function (res) {
              ...
              // return (emoji.get(':smiley_cat:'), chalk.bold('Github Issue Title:'), titleOfIssue)
            });
          }
        }
      })
